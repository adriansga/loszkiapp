...
export default function CalendarWidget({ initialEvents }: { initialEvents: CalendarEvent[] }) {
  ...
  return (
    <Link href="/kalendarz" className="bg-white rounded-xl p-5 border border-zinc-200 shadow-sm hover:border-emerald-400 hover:shadow-md transition-all block">
      ...
      {events.length === 0 ? (
        <p className="text-sm text-zinc-400">Nic zaplanowanego na dziś → dodaj w kalendarzu</p>
      ) : (
            ...
            <div key={ev.id} className={`flex items-start gap-2.5 ${ev.is_done ? 'opacity-50' : ''}`}>
              <button
                ...
                className={`mt-0.5 w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${
                  ev.is_done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-zinc-300 hover:border-emerald-400'
                }`}
              >
                {ev.is_done && <span className="text-[10px]">✓</span>}
              </button>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${ev.is_done ? 'line-through text-zinc-400' : 'text-zinc-800'}`}>{ev.title}</p>
